...
#include "HingeBoard.hpp"
#include <math.h>

#include <iostream>
using namespace std;

HingeBoard::HingeBoard(b2World *world, Layer *parentCocosLayer, float worldToPixelScale, Vec2 offsetOriginToBeAdded, HingeBoardInitialParameters *initialParameters){
    ...
    this->scoreDeltaLabel->setPosition(Vec2(offsetOriginToBeAdded.x + Director::getInstance()->getVisibleOrigin().x + this->boardBody->GetPosition().x*worldToPixelScale, offsetOriginToBeAdded.y + Director::getInstance()->getVisibleOrigin().y + (this->boardBody->GetPosition().y + this->heightHalf + 0.15)*worldToPixelScale));
...
HingeBoard::~HingeBoard(){
    delete virtualRod;
}

//implement interface
void HingeBoard::updateState(float dt, float worldToPixelScale, cocos2d::Vec2 offsetOriginToBeAdded){
...
double HingeBoard::clip(double x){
...
long long HingeBoard::boardCollidedReturnScoreUpdate(b2Contact *contact, float worldToPixelScale, Vec2 offsetOriginToBeAdded, const vector<AbstractBall*>& balls_in_game){
...
void HingeBoard::showScoreUpdate(long long deltaScore, float worldToPixelScale, Vec2 offsetOriginToBeAdded){
